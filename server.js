...
app.post('/api/uploads', requireAuth, requireMember, async (req, res) => {
  ...
  try { const storagePath = process.env.SUPABASE_BUCKET && process.env.SUPABASE_SERVICE_ROLE_KEY ? `${req.user.id}/${Date.now()}-${crypto.randomBytes(8).toString('hex')}-${fileName.replace(/[^a-zA-Z0-9._-]/g, '_')}` : null; if (storagePath) await supabase.storageUpload(storagePath, data, mimeType); const result = await query('INSERT INTO attachments (uploader_id, file_name, mime_type, file_size, data, storage_path) VALUES (?, ?, ?, ?, ?, ?)', [req.user.id, fileName, mimeType, data.length, storagePath ? null : data, storagePath]); res.status(201).json({ id: result.insertId, fileName, mimeType, fileSize: data.length, url: `/api/uploads/${result.insertId}` }); } catch (error) { console.error(error); res.status(503).json({ error: 'Unable to upload file.' }); }
});
app.get('/api/uploads/:id', async (req, res) => { try { const [file] = await query('SELECT file_name, mime_type, data, storage_path FROM attachments WHERE id = ?', [req.params.id]); if (!file) return res.sendStatus(404); res.setHeader('Content-Type', file.mime_type); res.setHeader('Content-Disposition', `inline; filename="${file.file_name.replace(/"/g, '')}"`); if (file.storage_path) { const stored = await supabase.storageDownload(file.storage_path); return res.send(Buffer.from(await stored.arrayBuffer())); } res.send(file.data); } catch { res.sendStatus(404); } });
...
app.get('/api/voice/config', requireAuth, (_req, res) => { let iceServers = [{ urls: 'stun:stun.l.google.com:19302' }]; try { if (process.env.ICE_SERVERS_JSON) iceServers = JSON.parse(process.env.ICE_SERVERS_JSON); } catch {} res.json({ iceServers }); });
