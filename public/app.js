...
async function sendVoiceSignal(recipientId, payload) { await fetch(`/api/voice/${state.voiceChannel.id}/signals`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ recipientId, payload }) }); }
function voicePeer(id, initiator) { if (state.peers.has(id)) return state.peers.get(id); const peer = new RTCPeerConnection({ iceServers: [{ urls: 'stun:stun.l.google.com:19302' }] }); state.mediaStream?.getTracks().forEach((track) => peer.addTrack(track, state.mediaStream)); peer.onicecandidate = (event) => event.candidate && sendVoiceSignal(id, { type: 'ice', candidate: event.candidate }); peer.ontrack = (event) => { let tile = document.querySelector(`#participant-${id}`); if (!tile) { tile = document.createElement('div'); tile.className = 'video-tile'; tile.id = `participant-${id}`; document.querySelector('#video-grid').append(tile); } tile.innerHTML = ''; tile.append(Object.assign(document.createElement('audio'), { autoplay: true, controls: false, srcObject: event.streams[0] })); }; peer.onconnectionstatechange = () => { if (['failed', 'closed', 'disconnected'].includes(peer.connectionState)) { peer.close(); state.peers.delete(id); document.querySelector(`#participant-${id}`)?.remove(); } }; state.peers.set(id, peer); if (initiator) peer.createOffer().then((offer) => peer.setLocalDescription(offer).then(() => sendVoiceSignal(id, { type: 'offer', offer }))); return peer; }
async function handleVoiceSignal(signal) { const sender = Number(signal.sender_id); const payload = typeof signal.payload === 'string' ? JSON.parse(signal.payload) : signal.payload; if (sender === Number(state.user.id)) return; if (payload.type === 'join') { voicePeer(sender, true); return; } const peer = voicePeer(sender, false); if (payload.type === 'offer') { await peer.setRemoteDescription(payload.offer); const answer = await peer.createAnswer(); await peer.setLocalDescription(answer); await sendVoiceSignal(sender, { type: 'answer', answer }); } else if (payload.type === 'answer') await peer.setRemoteDescription(payload.answer); else if (payload.type === 'ice' && payload.candidate) await peer.addIceCandidate(payload.candidate); }
async function pollVoiceSignals() { if (!state.voiceChannel) return; const response = await fetch(`/api/voice/${state.voiceChannel.id}/signals?after=${state.voiceCursor}`); if (!response.ok) return; const signals = await response.json(); for (const signal of signals) { state.voiceCursor = Math.max(state.voiceCursor, Number(signal.id)); await handleVoiceSignal(signal); } }
async function joinVoice(channel) { if (!navigator.mediaDevices?.getUserMedia) return showToast('Voice rooms require a secure browser connection'); try { state.voiceChannel = channel; state.mediaStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false }); document.querySelector('#voice-room-name').textContent = channel.name; document.querySelector('#voice-connection').textContent = 'Connected'; document.querySelector('#voice-modal').classList.add('open'); document.querySelector('#video-grid').innerHTML = '<div class="video-tile">Your microphone is connected</div>'; await sendVoiceSignal(null, { type: 'join' }); clearInterval(state.voicePoller); state.voicePoller = setInterval(pollVoiceSignals, 1200); await pollVoiceSignals(); } catch (error) { showToast(error.message || 'Unable to access microphone'); } }
...
document.querySelector('#leave-voice').addEventListener('click', () => { clearInterval(state.voicePoller); state.peers.forEach((peer) => peer.close()); state.peers.clear(); state.mediaStream?.getTracks().forEach((track) => track.stop()); state.mediaStream = null; state.voiceChannel = null; document.querySelector('#voice-modal').classList.remove('open'); document.querySelector('#video-grid').innerHTML = ''; });
document.querySelector('#voice-mic').addEventListener('click', (event) => { if (!state.mediaStream) return; state.muted = !state.muted; state.mediaStream.getAudioTracks().forEach((track) => { track.enabled = !state.muted; }); event.currentTarget.setAttribute('aria-label', state.muted ? 'Unmute microphone' : 'Mute microphone'); showToast(state.muted ? 'Microphone muted' : 'Microphone on'); });
