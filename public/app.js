...
let liveRoom;
async function joinVoice(channel) { if (!window.LivekitClient) return showToast('LiveKit is still loading'); try { const response = await fetch('/api/livekit/token', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ channelId: channel.id }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); liveRoom?.disconnect(); liveRoom = new LivekitClient.Room(); document.querySelector('#voice-room-name').textContent = channel.name; document.querySelector('#voice-connection').textContent = 'Connecting…'; document.querySelector('#voice-modal').classList.add('open'); liveRoom.on(LivekitClient.RoomEvent.TrackSubscribed, (track, publication, participant) => { const tile = document.createElement('div'); tile.className = 'video-tile'; tile.id = `participant-${participant.identity}`; tile.textContent = participant.identity; const media = track.attach(); tile.innerHTML = ''; tile.append(media); document.querySelector('#video-grid').append(tile); }); liveRoom.on(LivekitClient.RoomEvent.TrackUnsubscribed, (track, publication, participant) => { track.detach().forEach((element) => element.remove()); document.querySelector(`#participant-${participant.identity}`)?.remove(); }); await liveRoom.connect(data.url, data.token); await liveRoom.localParticipant.setMicrophoneEnabled(true); document.querySelector('#voice-connection').textContent = 'Connected'; document.querySelector('#video-grid').innerHTML = '<div class="video-tile">Your camera is off</div>'; } catch (error) { document.querySelector('#voice-modal').classList.remove('open'); showToast(error.message || 'Unable to join voice room'); } }
...
document.querySelector('#leave-voice').addEventListener('click', () => { liveRoom?.disconnect(); document.querySelector('#voice-modal').classList.remove('open'); document.querySelector('#video-grid').innerHTML = ''; });
document.querySelector('#voice-mic').addEventListener('click', async (event) => { if (!liveRoom) return; const enabled = !liveRoom.localParticipant.isMicrophoneEnabled; await liveRoom.localParticipant.setMicrophoneEnabled(enabled); event.currentTarget.setAttribute('aria-label', enabled ? 'Mute microphone' : 'Unmute microphone'); showToast(enabled ? 'Microphone on' : 'Microphone muted'); });
document.querySelector('#voice-camera').addEventListener('click', async (event) => { if (!liveRoom) return; const enabled = !liveRoom.localParticipant.isCameraEnabled; await liveRoom.localParticipant.setCameraEnabled(enabled); event.currentTarget.setAttribute('aria-label', enabled ? 'Turn camera off' : 'Turn camera on'); showToast(enabled ? 'Camera on' : 'Camera off'); });
